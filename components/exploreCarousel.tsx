...
const ExploreCarousel = ({ productImages }: ExploreCarouselProps) => {
  ...
  const extendedImages = hasAtLeastOne
    ? [
        productImages[productImages.length - 1], // The last image (for looping)
        ...productImages, // Add all product images
        productImages[0], // The first image (to loop back)
      ]
    : [];
  ...
  const handleTransitionEnd = () => {
    ...
    if (current === maxIndex) {
      setIsAnimating(false);
      setCurrent(1); // Loop back to the first image
      setTimeout(() => setIsAnimating(true), 50);
    } else if (current === 0) {
      setIsAnimating(false);
      setCurrent(productImages.length); // Loop to the last image
      setTimeout(() => setIsAnimating(true), 50);
    }
  };
  ...
  const nextSlide = () => {
    ...
    setCurrent((prev) => {
      if (prev >= maxIndex) {
        setIsAnimating(false);
        setCurrent(1); // Jump to the first image
        setTimeout(() => setIsAnimating(true), 50);
        return prev;
      }
      return prev + 1;
  ...
  const prevSlide = () => {
    ...
    setCurrent((prev) => {
      if (prev <= 0) {
        setIsAnimating(false);
        setCurrent(productImages.length); // Jump to the last image
        setTimeout(() => setIsAnimating(true), 50);
        return prev;
      }
      return prev - 1;
